...
def multipleWellZPlots(self : 'Plot'):
    '''
   Multiple Cross plot with Z axis for each well in Well_List
    '''
    ...
    for well in self.wellList.GetWells():
        ...
        try:
            self.points[fig.number] = self.axes[ax_idx].scatter(c1, c2, c=zc, marker='o', cmap=zcolmap, vmin=zc_scale.min, vmax=zc_scale.max)           # Xplot  pts is needed to connect to colorbar
        except  Exception:
        ...
        self.axes[ax_idx].set(title = 'Z-plot of '+ crv2 +' versus ' + crv1)
        ...
        cb = fig.colorbar(self.points[fig.number],ax=self.axes[ax_idx],cax=cax)
        ...
        if crv2 in marklist:
            self.axes[ax_idx+2].scatter(c1, c1.index, color=self.settings.Get('c2_col'), marker='o')
        else:
        ...
        if overlayImageFile != '':
            ...
            self.axes[ax_idx].imshow(img, extent= [c1_scale.min,c1_scale.max,c2_scale.min,c2_scale.max])                  #, extent=[-5, 80, -5, 30]
